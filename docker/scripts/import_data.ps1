...
foreach ($f in $csvFiles) {
    ...
    if (-not (Test-Path $fullPath)) {
        Write-Warning "ERREUR : $f introuvable dans $DATA_DIR"
    }
    Write-Host "  OK : $f"
}
...
# \r\n = CRLF Windows ; les CSV sont copiés via docker cp depuis le poste Windows
...
if ($LASTEXITCODE -ne 0) { Write-Warning "MySQL exit code $LASTEXITCODE (WARNING non bloquant, ex. ERROR 1262)" }
